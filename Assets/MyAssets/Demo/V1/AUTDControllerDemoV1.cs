...
public class AUTDControllerDemoV1 : MonoBehaviour
{
    ...
    //Determine one circle
    float centorCirclesPosX = 0f;
    float circlePosX;
    public float CirclePosX
    {
        get { return this.circlePosX; }
        private set { this.circlePosX = value; }
    }
    List<float> eachCirclePosX = new List<float>();
    int numberPlacement;
    ...
    void Update()
    {
        ...
        //Determine circlePosX
        numberPlacement=_manager.NumberPlacement;
        circlePosX = FindTargetCirclePosX(eachCirclePosX, fingerPosX);
    ...
    private float FindTargetCirclePosX(List<float> list, float value)
    {
        list.Clear();
        for (int i = 0; i <= numberPlacement; i++)
        {
            list.Add(0);
            list.Add((i * curveDistance)); //0.01f<=RightBump.transform.position.x
            list.Add(-(i * curveDistance)); //-0.01f<=LeftBump.transform.position.x
        }
    ...
    private void ConvexPosition()
    {
        for (var i = 0; i < Size; i++)
        {
            ...
            if (Mathf.Abs(fingerPosX - circlePosX) <= halfCurveLength / delay) //When Curved
            {
                ...
                focusPosX = delay * (fingerPosXInLoop - circlePosX) + circlePosX;
                focusPosY = fingerRadius - Mathf.Sqrt(Mathf.Pow(fingerRadius, 2) - Mathf.Pow(fingerPosXInLoop - focusPosX, 2));
    ...
    private void ConvexStrength()
    {
        for (var i = 0; i < Size; i++)
        {
            ...
            if (Mathf.Abs(fingerPosX - circlePosX) <= halfCurveLength / delay) //When Curved
            {
                ...
                fingerPosXInLoop = fingerPosX;
                focusPosX = delay * (fingerPosXInLoop - circlePosX) + circlePosX;
                phiRad = Mathf.Atan2(Mathf.Sqrt(Mathf.Pow(curveRadius, 2) - Mathf.Pow(focusPosX - circlePosX, 2)), focusPosX - circlePosX);
