...
public class AUTDController1CTE : MonoBehaviour
{
    ...
    const int Size = 40;   // freq*size   sould be < 1000 
    ...
    private void ExecutePosition()
    {
        for (var i = 0; i < Size; i++)
        {
            ...
            if (Mathf.Abs(fingerPosX * delay) <= halfCurveLength) //When Curved
            {
                ...
                focusPosY = fingerRadius - Mathf.Sqrt(Mathf.Pow(fingerRadius, 2) - Mathf.Pow(fingerPosXInLoop * (delay - 1), 2));
